    ...
    return (
        <div className="bg-white p-4 shadow-md shadow-black-500 rounded-lg">
            {storeCategoryDescription.map((item, index) => (
                <div key={index} className="pb-4 pr-4 flex gap-2 justify-between">
                    ...
                    <div className="w-full">
                        ...
                        {item.detail_category_description_content && !hasYoutubeIframe(item.detail_category_description_content) && (
                            <div className="">
                                <p
                                    className={`${item.detail_category_description_img_url?.length > 0 ? 'bg-[#7864F9] text-white px-2 py-4 text-lg rounded-xl' : 'py-2 font-medium'}`}
